...
CREATE OR ALTER PROC sp_ADMIN_LOADTABLE
	@ID_CANVA VARCHAR(5),
    @ResponseMessage NVARCHAR(255) OUTPUT
AS
BEGIN
    SELECT VATF.X_COORDINATE, VATF.Y_COORDINATE, VATF.WIDTH, VATF.HEIGHT, VATF.RADIUS, 
		VATF.ID_FOOD, VATF.FOOD_NAME, VATF.FOOD_TYPENAME, VATF.AMOUNT_IN_TABLE, 
		VATF .PRICE, VATF.FOOD_TYPE_STATUS, VATF .IMAGE_LINK,
        VATF.TABLE_STATUS, VATF.ID_TABLE, VATF.SHAPE_TYPENAME
    FROM V_ADMIN_TABLE_FOODs VATF
    WHERE VATF.ID_CANVA = @ID_CANVA
END
GO
